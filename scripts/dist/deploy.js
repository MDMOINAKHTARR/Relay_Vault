...
async function main() {
    ...
    console.log("Account balance:", (await hardhat_1.ethers.provider.getBalance(deployer.address)).toString());
    // 1. Deploy AgentRegistry
    console.log("\n[1/5] Deploying AgentRegistry...");
    const Registry = await hardhat_1.ethers.getContractFactory("AgentRegistry");
    ...
    const Negotiation = await hardhat_1.ethers.getContractFactory("NegotiationEngine");
    ...
    const Escrow = await hardhat_1.ethers.getContractFactory("TaskEscrow");
    ...
    const Resolver = await hardhat_1.ethers.getContractFactory("DisputeResolver");
    ...
    const BondManager = await hardhat_1.ethers.getContractFactory("ReputationBondManager");
    ...
    // 7. Auto-write addresses to .env.local so the frontend picks them up instantly
    ...
    console.log("=========================================");
    console.log("\n✅ RelayVault protocol is LIVE on Monad Testnet!");
    console.log("Update contracts.ts with these addresses to activate the frontend.\n");
}
main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
